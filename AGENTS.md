...
| 步骤 | 做法 |
|---|---|
| 领任务 | 只做带 `for-mavis` 标签的 Issue。开工时在 Issue 下留言"认领"。 |
| 建分支 | 从最新 `main` 切出 `mavis/<issue号>-<短名>`，例如 `mavis/12-day-report`。 |
| 提交 | 小步提交，提交说明用中文或英文均可，写清"做了什么、为什么"。 |
| 提 PR | 目标分支 `main`。标题引用 Issue（`#12 值班日报`）。正文按第 6 节模板填写。 |
| 审查 | 由 Claude 审查。收到 review 后逐条修改或回复理由，改完 push 到同一分支，并在 PR 下留言"已按审查修改"。 |
| 合并 | 审查通过（Approve）后由仓库主人合并。**不要自己合并，不要强推 `main`。** |

**范围纪律**：只改 Issue 要求的内容。发现别的问题，在 PR 正文"顺带发现"里写出来，不要顺手改。
